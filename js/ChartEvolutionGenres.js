...
ChartEvolutionGenres.prototype.updateOverlay = function() {
	...
	this.overlay = this.svg.append('foreignObject')
		.attr('id', 'overlay')
		.append('div');

	switch (this.genre) {
		case 'Drama':
			movies = ['Le pont de la rivière Kwaï', 'Les dents de la mer', 'Madame Doubtfire', 'The Dark Knight: Le chevalier noir'];
			yPosition = 0.1;
			data = that.audiencesData;
			originalData = that.originalAudiencesData;
			break;
		case 'Romance':
			movies = ['Grease', 'Titanic', 'Twilight: Chapitre 1 - Fascination'];
			yPosition = 0.6;
			data = that.audiencesData;
			originalData = that.originalAudiencesData;
			break;
		case 'Action':
			movies = ['Gladiator', 'Le seigneur des anneaux: Le retour du roi', 'Mad Max: Fury Road'];
			yPosition = 0.8;
			data = that.oscarsData;
			originalData = that.originalOscarsData;
			break;
		case 'Adventure':
			movies = ['Les dents de la mer', 'Inception', "L'odyssée de Pi"];
			yPosition = 0.8;
			data = that.oscarsData;
			originalData = that.originalOscarsData;
			break;
	...
	for (let title of movies) {
		...
		let movie = originalData.map(function(e) {
			return e.values;
		})
		.flat()
		.find(function(e) {
		...
		let frequency = data.find(function(e) {
			return e.key == movie.year.getFullYear();
		})
		...
		//     .attr('x', that.xScale(movie.year.getFullYear()) + that.xScale.bandwidth() / 2)
		//     .attr('y', that.yScale(yPosition))
		// 	.attr('text-anchor', 'middle')
		//  	.text(movie.title)
		//  	.style('opacity', 0)
		//  	.transition()
		...
		//     .attr('marker-end', 'url(#triangle)')
		//     .attr('x1', that.xScale(movie.year.getFullYear()) + that.xScale.bandwidth() / 2)
		//     .attr('y1', function() {
		//     	return (yPosition < frequency) ? that.yScale(yPosition) - 10 : that.yScale(yPosition) + 10;
		//     })
		//     .attr('x2', that.xScale(movie.year.getFullYear()) + that.xScale.bandwidth() / 2)
		//     .attr('y2', function() {
		//     	return (yPosition > frequency) ? that.yScale(frequency) - 10 : that.yScale(frequency) + 10;
		//     })
		//     .attr('stroke', 'blue')
		//     .attr('stroke-width', 3)
		//     .style('opacity', 0)
